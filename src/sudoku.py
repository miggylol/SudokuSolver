...
class Cell(object):
	...
	def __init__(self,row,column):  # Creates a cell object and gives it position on the board
		self.row = row
		self.column = column
		self.number = 0
		self.possible_numbers = {} # you can also put set()
		self.exhausted = False  # If you have tried all the possible numbers... It's false because we haven't

	def try_new_number(self):

		self.number = self.possible_numbers.pop()  # Taking the number out of the possible numbers list and puts it in number

		if len(self.possible_numbers) == 0: # after popping possible numbers enough, you are going to run out of numbers to pop
			self.exhausted = True  # Tells us that there no more numbers available

	def reset(self): # Resets the whole board
	...
	def set_possible_numbers(self, numbers): # Takes numbers from outside
...
class Board(object):
	...
	def get_column(self,column):
		# get a coumn of numbers for the specified column
		column_numbers = []
		for row in range (9):
			col = self.board[row][column] # What does this mean/do???
			num = col.number
...
# What does this range mean?
	...
	def is_valid(self):
		...
		square_valid = True
		# range (0,9,3) staeps through the numbers
	...
	def solve(self):
		""" Solve a 9x9 Sudoky Puzzle by using Brute Force(backtracking)."""

		#Checks if the initial board is valid
		if self.is_valid():
			attempted_cells = []
			row = 0
			start = time() # Where is time function located
			attempted_cells = []
			while row < 9:
				column = 0
				while column < 9:
					...
					backtracking = False
					if not cell.number: # if the cells nymber is 0
			...
			return True, end - start # Check to see where "end" is
